...
class Api(object):
  ...
  def GetDirectMessages(self,
    ...
    Returns:
      A tuple with:
         1. A sequence of twitter.DirectMessage instances
         2. Response headers
    '''
    if not self.__auth:
      raise TwitterError({'message': "The twitter.Api instance must be authenticated."})
    ...
    try:
        # intercept headers
        response_headers = json.headers
    except Exception, err:
        raise TwitterError({'message': "cannot retrieve response headers - {}".format(str(err))})

    return ([DirectMessage.NewFromJsonDict(x) for x in data], response_headers)
